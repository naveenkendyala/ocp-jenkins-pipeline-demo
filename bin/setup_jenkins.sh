...
#oc import-image openshift/jenkins-agent-maven:v4.0 --from=registry.redhat.io/openshift4/ose-jenkins-agent-maven --confirm  -n ${GUID}-jenkins
#oc import-image openshift4/ose-jenkins-agent-maven --from=registry.redhat.io/openshift4/ose-jenkins-agent-maven --confirm  -n ${GUID}-jenkins
...
oc create secret generic gitea-secret --from-literal=username=srinivas.sarat.yadavalli-ibm.com --from-literal=password=Adm1n1strat0r
#oc set build-secret --source bc/tasks gitea-secret

sleep 60

oc new-app --template=eap72-basic-s2i --param APPLICATION_NAME=tasks --param SOURCE_REPOSITORY_URL=https://homework-gitea.apps.shared-na4.na4.openshift.opentlc.com/SaratHW/openshift-tasks-private.git --param SOURCE_REPOSITORY_REF=master --param CONTEXT_DIR=/ --param MAVEN_MIRROR_URL=https://homework-nexus-registry.apps.shared-na4.na4.openshift.opentlc.com/repository/maven-all-public
